    ...
    private function sendConfirmationEmailAsync($emailData) {
        ...
        register_shutdown_function(function() use ($emailData) {
            ...
            if (file_exists($templatePath)) {
                ...
                $serverHost = 'localhost'; // Default fallback
                
                // Method 1: Try to get actual local IP from system (Windows)
                if (strtoupper(substr(PHP_OS, 0, 3)) === 'WIN') {
                    // Use ipconfig on Windows to get the actual IP
                    $output = @shell_exec('ipconfig | findstr /R /C:"IPv4 Address"');
                    if ($output) {
                        // Extract IP addresses from output
                        preg_match_all('/\d+\.\d+\.\d+\.\d+/', $output, $matches);
                        if (!empty($matches[0])) {
                            // Get the last IP (usually the active network adapter)
                    ...
                    // Unix/Linux/Mac - use hostname -I or ip addr
                    $output = @shell_exec("hostname -I 2>/dev/null || ip addr show | grep 'inet ' | grep -v '127.0.0.1' | awk '{print $2}' | cut -d/ -f1 | head -n1");
                    if ($output) {
                ...
                // Method 2: Try SERVER_ADDR if shell method didn't work
                if ($serverHost === 'localhost' && isset($_SERVER['SERVER_ADDR']) && 
                    $_SERVER['SERVER_ADDR'] !== '::1' && $_SERVER['SERVER_ADDR'] !== '127.0.0.1') {
                    $serverHost = $_SERVER['SERVER_ADDR'];
                }
                
                // Method 3: Try hostname resolution as last resort
                if ($serverHost === 'localhost') {
                    $hostname = @gethostname();
                    if ($hostname !== false) {
                        $localIP = @gethostbyname($hostname);
                        if ($localIP !== $hostname && filter_var($localIP, FILTER_VALIDATE_IP, FILTER_FLAG_IPV4)) {
                            $serverHost = $localIP;
                        }
                    }
                }
                
                error_log("EMAIL DEBUG: Detected server host: " . $serverHost);
