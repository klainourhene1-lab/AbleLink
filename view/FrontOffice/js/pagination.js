...
function renderPaginationControls(gridId) {
    ...
    html += `
        <div class="pagination-goto">
            <label for="goto-${gridId}" class="pagination-goto-label">Aller à:</label>
            <input 
                type="number" 
                id="goto-${gridId}" 
                ...
                onkeypress="if(event.key === 'Enter') goToPage('${gridId}')"
            />
            <button class="pagination-goto-btn" onclick="goToPage('${gridId}')">
                Go
            </button>
        </div>
    `;
...
function goToPage(gridId) {
    const input = document.getElementById(`goto-${gridId}`);
    if (!input) return;
    ...
    if (isNaN(pageNumber) || pageNumber < 1 || pageNumber > totalPages) {
        input.value = '';
        input.placeholder = paginationState[gridId].currentPage;
