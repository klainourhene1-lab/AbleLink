...
function openEventDetails(eventId) {
    ...
    const isEventCreator = event.createur_id === window.CURRENT_USER_ID || event.idUtilisateur === window.CURRENT_USER_ID;
    ...
    document.getElementById('detailsContent').innerHTML = `
        <div style="display: grid; gap: 20px;">
            ...
            <div class="card-actions">
                ${!isUserParticipating && event.statut === 'Publié' && !isEventPassed(event) && !isFull && canJoinOrEvaluate ? 
                    `<button class="cta-button primary" onclick="registerForEvent('${event.id_evenement}'); closeDetailsModal();">S'inscrire</button>` : ''}
                    
                ${isUserParticipating && isEventPassed(event) && canJoinOrEvaluate ? 
...
function openEvaluationModal(eventId) {
    ...
    // Prevent companies and admins from evaluating events they created
    if ((isCompany || isAdmin) && isEventCreator) {
